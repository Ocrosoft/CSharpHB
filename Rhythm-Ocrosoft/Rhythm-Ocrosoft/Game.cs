    ...
    class Game
    {
        ...
        public void Draw(Graphics g)
        {
            try
            {
                ...
                //曲名，制作者
                if (_openSong != "" && _openSong != null && (_gameState == GameState.Play || _gameState == GameState.Pause))
                {
                    g.DrawString(_openSong, fontSong, fontBrush, new Point((int)((1024 / 2.0 - _openSong.Length / 2.0 * 12) * _zoomRate), (int)(290 * _zoomRate)));
                    g.DrawString(_maker, fontSong, fontBrush, new Point((int)((1024 / 2.0 - _maker.Length / 2.0 * 14) * _zoomRate), (int)(335 * _zoomRate)));
                }
                //百分比
                if (_gameState == GameState.Play)
                {
                    double rate = ((Accept - Perfect) * 0.8 + Perfect * 1.0) / KeyCount * 100;
                    if ((int)rate == rate)
                    {
                        if (rate < 10)
                            g.DrawString(((rate.ToString().Substring(0, 1)) + ".00000").Substring(0, 4) + "%", new Font("Comic Sans MS", (float)(20 * _zoomRate)), fontBrush, (int)((1024 - 100) * _zoomRate), (int)(10 * _zoomRate));
                        else
                            g.DrawString(((rate.ToString().Substring(0, 2)) + ".00000").Substring(0, 5) + "%", new Font("Comic Sans MS", (float)(20 * _zoomRate)), fontBrush, (int)((1024 - 100) * _zoomRate), (int)(10 * _zoomRate));
                    }
                    else
                    {
                        if (rate == 0)
                            g.DrawString("0.00%", new Font("Comic Sans MS", (float)(20 * _zoomRate)), fontBrush, (int)((1024 - 100) * _zoomRate), (int)(10 * _zoomRate));
                        else if (rate < 10)
                            g.DrawString(((rate.ToString()) + "00000").Substring(0, 4) + "%", new Font("Comic Sans MS", (float)(20 * _zoomRate)), fontBrush, (int)((1024 - 100) * _zoomRate), (int)(10 * _zoomRate));
                        else
                            g.DrawString(((rate.ToString()) + "00000").Substring(0, 5) + "%", new Font("Comic Sans MS", (float)(20 * _zoomRate)), fontBrush, (int)((1024 - 100) * _zoomRate), (int)(10 * _zoomRate));
                    }
                }
                /*
                //显示连击，黑色
                ...
                {
                    if (_combo < 10)
                        g.DrawString(_combo.ToString(), myFont, new SolidBrush(Color.Black), (int)((1024 - 80) * _zoomRate), (int)((768 / 2.0 - 90) * _zoomRate));
                    else if (_combo < 100)
                        g.DrawString(_combo.ToString(), myFont, new SolidBrush(Color.Black), (int)((1024 - 90) * _zoomRate), (int)((768 / 2.0 - 90) * _zoomRate));
                    else
                        g.DrawString(_combo.ToString(), myFont, new SolidBrush(Color.Black), (int)((1024 - 105) * _zoomRate), (int)((768 / 2.0 - 90) * _zoomRate));
                }
                */
                //绘制按键
        ...
        public void Open(string fileName)
        {
            try
            {
                ...
                //int pattern = binaryReader.ReadInt32();//ver1.1已删除
                _difficulty = binaryReader.ReadInt32();
                string description = binaryReader.ReadString();//ver1.1增加，描述，但暂时无用
                KeyCount1 = binaryReader.ReadInt32();
                _keyCount = 0;
        ...
        public void JudgeKeyPress(char keyChar, Graphics g)
        {
            try
            {
                for (int i = 0; i < _listDrKey.Count; i++)
                {
                    ...
                    {
                        if (keys.Position.Y > 710 * _zoomRate) continue;//超过线的可见但不可触发
